// GWCFC Radar - Discord to radar chat bridge (Cloudflare Worker)
// ---------------------------------------------------------------------------
...
//
// So instead this Worker wakes on a timer, asks Discord's REST API "anything
...
// Cloudflare runs it; there is nothing to keep alive.
//
// The trade-off is latency: Cloudflare's shortest cron interval is one minute,
// so a Discord message takes up to ~60s to appear on the map. Messages going
// the other way are instant, because those do not go through here at all.
//
...
export default {
  // Cloudflare's shortest cron is one minute; that is the latency floor here.
  ...
  // GET /health  - is it configured and can it reach Discord and Firestore
